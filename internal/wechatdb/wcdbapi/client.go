...
func (c *Client) ensureDecrypted(src string) (string, error) {
	...
	if walMTime > 0 {
		if err := applyWAL(walPath, tmpPath, src, useKey); err != nil {
			_ = os.Remove(tmpPath)
			return "", err
		}
	}
...
func applyWAL(walPath, outPath, srcDBPath, hexKey string) error {
	...
	// Read salt from the source encrypted database file
	salt, err := readDBSalt(srcDBPath)
	if err != nil {
		return fmt.Errorf("read db salt for WAL: %w", err)
	}

	// Derive keys using PBKDF2 (SQLCipher 4 standard)
	...
	for pos := 0; pos+frameSize <= len(frames); pos += frameSize {
		...
		dec, err := decryptWALPage(encKey, macKey, pageBuf, pg)
		if err != nil {
			return err
		}
...
// readDBSalt reads the first 16 bytes (salt) from an encrypted database file.
...
func decryptWALPage(encKey, macKey []byte, pageData []byte, pgno uint32) ([]byte, error) {
	// WAL pages are treated as non-first pages (no salt/header special logic)
	return decryptPageRaw(encKey, macKey, pageData, int(pgno))
}

func decryptPageRaw(encKey, macKey []byte, pageData []byte, pgno int) ([]byte, error) {
	if len(pageData) < pageSize || len(encKey) != 32 {
		return nil, fmt.Errorf("invalid page or key")
	}

	// Verify HMAC before decryption
	if len(macKey) == 32 {
		if !verifyPageHMACRaw(pageData, macKey, pgno) {
			return nil, fmt.Errorf("HMAC verification failed for page %d", pgno)
		}
	}
	...
	if pgno == 1 {
		...
		dec, err := aesCBCDecryptRaw(encKey, iv, enc)
		if err != nil {
			return nil, err
		}
...
func verifyPageHMACRaw(pageData, macKey []byte, pgno int) bool {
	...
	dataEnd := pageSize - reserveSize + 16 // IVSize = 16
